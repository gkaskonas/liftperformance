                ...
                <div className="mb-16 flex flex-wrap" key={index}>
                    <div className="mb-6 w-full shrink-0 grow-0 basis-auto lg:mb-0 lg:w-6/12 lg:pr-6">
                        <div
                            ...
                            <Image src={blog.coverImage.url} width={blog.coverImage.width} height={blog.coverImage.height} className="w-full transition-opacity opacity-0 duration-[0.5s]" alt="Blog Cover Image" priority onLoadingComplete={(image) => image.classList.remove('opacity-0')} />
                    ...
                    <div className="w-full shrink-0 grow-0 basis-auto lg:w-6/12 lg:pl-6">
                        <Link href={`/blog/${blog.slug}`} className="mb-4 text-2xl font-bold"><h2>{blog.title}</h2></Link>
